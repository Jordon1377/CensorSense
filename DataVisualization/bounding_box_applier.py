...
#Function that Draws bounding boxes on a copy of an image and returns it. Uses Annotation and File directory for image path.
def DrawBBox(annotation: Annotation, directory: str):
    image_path = os.path.normpath(os.path.join(directory, annotation.path_name)) #Load path
    ...
    for box in annotation.boxes:
        cv2.rectangle(image, (box.x1, box.y1), (box.x1 + box.w, box.y1 + box.h), (0, 255, 0), 2)  # Draw green rectangle
    return image

#Function that outputs image to a folder
def SaveImageToFolder(image, folder_path: str = "DataVisualization/BoundingBoxVisualization", filename: str = "image.jpg"):
...
def Interface():
    ...
    # Step 1: Get the annotations file path from user
    ...
    # Step 2: Get choice to use all or random images
    ...
    if use_random == 'y':
        ...
        annotations = random.sample(annotations, min(x, len(annotations)))  # Ensure we don't exceed available images
    
    # Step 3: Get choice to display or save images
    action = input("Do you want to display images in a window or save them to a folder? (display/save): ").strip().lower()
    ...
    for annotation in annotations:
        image = DrawBBox(annotation, folder_path)
